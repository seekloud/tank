...
trait FpsComponentsDrawUtil{ this:GameContainerClientImpl =>
  ...
  protected def renderFps(networkLatency: Long,dataSize:Double) = {
    addFps()
    if(isRenderFps){
...
//      ctx.setTextAlign(TextAlignment.JUSTIFY)
      ...
      ctx.fillText(s"data: ${{dataSize/1024}.formatted("%.2f")}kb/s",canvasBoundary.x * canvasUnit - fpsString.length * canvasUnit/1.5,(canvasBoundary.y - LittleMap.h - 4) * canvasUnit)
