...
import com.neo.sk.tank.core.bot.BotActor._
...
trait BotControl {
  implicit val gameContainerOpt : Option[GameContainerServerImpl]
...
//          log.debug(s"${userActor.path} begin to do to eat the prop ${turnMsg}")
  ...
  private def userKeyDown(tankId:Int,keyCode:Int) = {
    val preExecuteAction = TankGameEvent.UserPressKeyDown(tankId, gameContainerOpt.get.systemFrame, keyCode, getActionSerialNum)
