...
object Route {
  def getUserJoinGameWebSocketUri(name:String, domain:String, playerInfo:PlayerInfo, roomIdOpt:Option[String]): String = {
    val wsProtocol = "ws"
    s"$wsProtocol://${domain}/tank${wsUserJoinGameUrl(URLEncoder.encode(name, "utf-8"),playerInfo.playerId, URLEncoder.encode(playerInfo.nickName,"utf-s"), playerInfo.accessCode, roomIdOpt)}"
